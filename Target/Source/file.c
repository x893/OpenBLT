...
static uint8_t     FileLibHexStringToByte(const char *hexstring);
	...
	static char FileLibByteNibbleToChar(uint8_t nibble);
	static char *FileLibByteToHexString(uint8_t byte_val, char *destination);
	static char *FileLibLongToIntString(uint32_t long_val, char *destination);
#else
	#define FileLibLongToIntString(v, d)
	#define FileLibByteToHexString(v, d)
#endif

/****************************************************************************************
* Hook functions
****************************************************************************************/
extern bool FileIsFirmwareUpdateRequestedHook(void);
extern const char *FileGetFirmwareFilenameHook(void);
extern void FileFirmwareUpdateStartedHook(void);
extern void FileFirmwareUpdateCompletedHook(void);
extern void FileFirmwareUpdateErrorHook(uint8_t error_code);

#if (BOOT_FILE_LOGGING_ENABLE > 0)
	extern void FileFirmwareUpdateLogHook(const char *info_string);
#else
	#define FileFirmwareUpdateLogHook(m)
#endif
...
** \return    true when in idle state, false otherwise.
**
****************************************************************************************/
bool FileIsIdle(void)
{
...
** \return    true when a firmware update is requested, false otherwise.
**
****************************************************************************************/
bool FileHandleFirmwareUpdateRequest(void)
{
	...
	if (FileIsFirmwareUpdateRequestedHook() == true)
	{
...
void FileTask(void)
{
	...
	else if (firmwareUpdateState == FIRMWARE_UPDATE_STATE_ERASING)
	{
		...
		{
			parse_result = FileSrecParseLine(lineParseObject.line, &lineParseObject.address, NULL);
		...
		if (f_eof(&fatFsObjects.file) > 0)
		{
			...
			FileLibByteToHexString((uint8_t)(eraseInfo.start_address >> 24), &loggingStr[0]);
			FileLibByteToHexString((uint8_t)(eraseInfo.start_address >> 16), &loggingStr[2]);
			FileLibByteToHexString((uint8_t)(eraseInfo.start_address >> 8), &loggingStr[4]);
			FileLibByteToHexString((uint8_t)eraseInfo.start_address, &loggingStr[6]);
			FileFirmwareUpdateLogHook(loggingStr);
			FileFirmwareUpdateLogHook("...");

			/* still here so we are ready to perform the memory erase operation */
			if (NvmErase(eraseInfo.start_address, eraseInfo.total_size) == false)
			{
				FileFirmwareUpdateLogHook("ERROR\n\r");
#if (BOOT_FILE_ERROR_HOOK_ENABLE > 0)
	...
	else if (firmwareUpdateState == FIRMWARE_UPDATE_STATE_PROGRAMMING)
	{
		...
		if (parse_result > 0)
		{
			...
			/* convert address to hex-string  */
			FileLibByteToHexString((uint8_t)(lineParseObject.address >> 24), &loggingStr[0]);
			FileLibByteToHexString((uint8_t)(lineParseObject.address >> 16), &loggingStr[2]);
			FileLibByteToHexString((uint8_t)(lineParseObject.address >> 8), &loggingStr[4]);
			FileLibByteToHexString((uint8_t)lineParseObject.address, &loggingStr[6]);
			FileFirmwareUpdateLogHook(loggingStr);
			FileFirmwareUpdateLogHook("...");

			/* program the data */
			if (NvmWrite(lineParseObject.address, parse_result, lineParseObject.data) == false)
			{
				FileFirmwareUpdateLogHook("ERROR\n\r");
...
**
****************************************************************************************/
SrecLineType_t FileSrecGetLineType(const char *line)
{
...
**
****************************************************************************************/
bool FileSrecVerifyChecksum(const char *line)
{
...
int16_t FileSrecParseLine(const char *line, uint32_t *address, uint8_t *data)
{
  ...
  /* verify the checksum */
  if (FileSrecVerifyChecksum(line) == false)
  {
...
static char *FileLibLongToIntString(uint32_t long_val, char *destination)
{
	uint32_t long_val_cpy = long_val;
